...
import {
    ...
    AlignMiddleLeft, AlignMiddleCenter, AlignMiddleRight,
    AlignBottomLeft, AlignBottomCenter, AlignBottomRight,
    Upload, Download, RefreshCw, XCircle, Trash2, Code, Move,
    Lock, Unlock, ChevronUp, ArrowLeft
} from './icons';
...
export const PRESET_BACKGROUNDS = [
    { name: 'Aura', url: 'https://media.discordapp.net/attachments/805778367706169415/1449361260532731967/Aura.png?ex=693e9e5c&is=693d4cdc&hm=5a83b23e95d270c14f976249e1b9603432b1fde13f1813e1a1261162921d719c&=&format=webp&quality=lossless&width=2315&height=1543' },
    { name: 'Ripple', url: 'https://media.discordapp.net/attachments/805778367706169415/1449361261157552168/Ripple.png?ex=693e9e5c&is=693d4cdc&hm=c615aeced90956f841230f53bfdc0209556705cda09853eeae5207762a184b9d&=&format=webp&quality=lossless&width=2315&height=1543' },
    { name: 'Abstract', url: 'https://images.unsplash.com/photo-1550684848-fac1c5b4e853?auto=format&fit=crop&w=600&q=80' },
    { name: 'Nebula', url: 'https://media.discordapp.net/attachments/805778367706169415/1449361262407323775/nebula.png?ex=693e9e5c&is=693d4cdc&hm=98c0a6a63bc02b4bb41405985603109f68b31c4466efe7d53e98439b5b24ce09&=&format=webp&quality=lossless&width=2315&height=1543' },
    { name: 'Nature', url: 'https://images.unsplash.com/photo-1472214103451-9374bd1c798e?auto=format&fit=crop&w=300&q=80' },
    { name: 'Meadow', url: 'https://media.discordapp.net/attachments/805778367706169415/1449361261673447475/Meadow.png?ex=693e9e5c&is=693d4cdc&hm=465e2bb60cb69d1dbb311e4b2969490f1ea4eb5899a3d8b924f2549a267881cd&=&format=webp&quality=lossless&width=2315&height=1543' },
];
            ...
            <div className="flex items-center space-x-2">
                ...
                <div className="flex-1">
                    <Slider
                        ...
                        onValueChange={(vals) => handleAngleChange(vals[0])}
                        min={0}
                        max={360}
                        step={1}
...
const SliderControl: React.FC<{
    ...
    <div className={`space-y-3 pt-1 ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
        <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-neutral-500 w-14 text-right tabular-nums">{value.toFixed(label === 'Scale' || label === 'Opacity' || step < 1 ? 2 : 0)}{unit}</span>
        </div>
        <Slider
            value={[value]}
...
const ALIGNMENT_OPTIONS: { value: Alignment; icon: React.FC<React.SVGProps<SVGSVGElement>> }[] = [
    { value: 'top-left', icon: AlignTopLeft }, { value: 'top-center', icon: AlignTopCenter }, { value: 'top-right', icon: AlignTopRight },
    { value: 'middle-left', icon: AlignMiddleLeft }, { value: 'middle-center', icon: AlignMiddleCenter }, { value: 'middle-right', icon: AlignMiddleRight },
    { value: 'bottom-left', icon: AlignBottomLeft }, { value: 'bottom-center', icon: AlignBottomCenter }, { value: 'bottom-right', icon: AlignBottomRight },
];

export const Controls: React.FC<ControlsProps> = (props) => {
    const {
        aspectRatio, setAspectRatio, gradient, setGradient,
        backgroundImage, handleBackgroundUpload, removeBackgroundImage,
        backgroundEffects, setBackgroundEffects,
        imageSettings, setImageSettings, uploadedImages, activeImageIndex,
        setActiveImageIndex, handleFileUpload, removeUploadedImage, removeAllUploadedImages,
        generateNewGradient, onDownloadSingle, onDownloadZip, isDownloading,
        setBackgroundImage,
    } = props;
        ...
        <div className="flex flex-col space-y-6 h-full">
                ...
                <AccordionItem value="foreground">
                    ...
                    <AccordionContent className="p-1">
                        ...
                        {uploadedImages.length > 0 ? (
                            <div className="space-y-6">
                                ...
                                <div className="space-y-6 pt-6 border-t border-neutral-800">
                                    ...
                                    <div className="space-y-6 rounded-lg bg-neutral-900/50 p-4">
                                        <div className="flex items-center justify-between">
                                            <label htmlFor="glass-toggle" className="text-sm font-medium text-neutral-300">Enable</label>
                                            <button
                                                role="switch"
                                        ...
                                        <div className={`flex items-center justify-between ${!imageSettings.glassmorphicBorder.enabled ? 'opacity-50 pointer-events-none' : ''}`}>
                                            ...
                                            <ColorPicker
                                                color={imageSettings.glassmorphicBorder.color}
                                                onChange={c => handleImageSettingChange('glassmorphicBorder', { ...imageSettings.glassmorphicBorder, color: c })}
                                            />
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div>
                                <button onClick={handleUploadClick} className="flex items-center justify-center space-x-2 w-full p-2.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-neutral-900 focus:ring-blue-500 font-medium">
                                    <Upload className="w-4 h-4" />
                        ...
                        <div className="space-y-4">
                            ...
                            {activeBackgroundTab === 'color' && (
                                <div className={`space-y-4 transition-opacity duration-300 ${backgroundImage ? 'opacity-30 pointer-events-none' : ''}`}>
                                    <button onClick={generateNewGradient} className="flex items-center justify-center space-x-2 w-full p-2.5 bg-neutral-800 hover:bg-neutral-700 rounded-lg transition-colors duration-200 text-neutral-300 font-medium border border-white/5">
                                    ...
                                    <div className="space-y-3 pt-2">
                                        ...
                                        <div className="grid grid-cols-4 gap-2">
                                            {PRESET_PALETTES.map(palette => (
                                                <button key={palette.name} onClick={() => setGradient(palette.gradient)} className="aspect-square w-full rounded-md border border-white/10 focus:outline-none ring-2 ring-transparent focus:ring-blue-500 hover:scale-105 transition-transform" style={{ background: `linear-gradient(${palette.gradient.angle}deg, ${palette.gradient.colors[0]}, ${palette.gradient.colors[1]})` }} title={palette.name} />
                                            ))}
                                        </div>
                                    </div>
                            ...
                            {activeBackgroundTab === 'image' && (
                                <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
                                    <button onClick={handleBackgroundUploadClick} className="flex items-center justify-center space-x-2 w-full p-2.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 rounded-lg transition-colors duration-200 font-medium border border-white/5">
                                        <Upload className="w-4 h-4" />
                                        <span>Upload Image</span>
                                    </button>
                    ...
                    <AccordionContent className="p-1">
                        <div className="space-y-6">
                            ...
                            <SliderControl
                                ...
                                value={backgroundEffects.patternOpacity}
                                onChange={v => handleBackgroundEffectChange('patternOpacity', v)}
                                min={0} max={1} step={0.01}
                                disabled={backgroundEffects.pattern === 'none'}
                            />
                        </div>
                    </AccordionContent>
                </AccordionItem>
            ...
            <div className="p-4 border-t border-neutral-800 space-y-4 bg-neutral-900/50 backdrop-blur-sm fixed bottom-0 left-0 right-0 z-50 lg:relative lg:bottom-auto lg:left-auto lg:right-auto lg:bg-transparent lg:border-none lg:p-0">
                ...
                <div className="flex space-x-3">
                    <button onClick={onDownloadSingle} disabled={isDownloading} className="flex-1 flex items-center justify-center space-x-2 p-3.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-blue-500/25 disabled:opacity-50 disabled:cursor-not-allowed">
                        {isDownloading ? (
                    ...
                    <div className="relative" ref={downloadMenuRef}>
                        <button
                            ...
                            className="h-full px-3 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 rounded-xl transition-colors border border-neutral-700"
                        >
                        ...
                        {isDownloadMenuOpen && (
                            <div className="absolute bottom-full right-0 mb-2 w-48 bg-neutral-900 border border-neutral-800 rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-2 fade-in duration-200">
                                <button onClick={() => { onDownloadZip(); setIsDownloadMenuOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-neutral-800 text-neutral-300 hover:text-white transition-colors flex items-center space-x-2">
