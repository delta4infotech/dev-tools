...
const SliderControl: React.FC<{
    ...
    <div className={`space-y-3 pt-1 ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
        <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-neutral-500 w-12 text-right tabular-nums">{value.toFixed(label === 'Opacity' || step < 1 ? 2 : 0)}{unit}</span>
        </div>
        <Slider
            value={[value]}
...
const StylePopover: React.FC<{
    ...
    const TabButton: React.FC<{ name: string; label: string }> = ({ name, label }) => (
        <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${activeTab === name ? 'bg-neutral-600 text-white' : 'text-neutral-400 hover:text-white'
                }`}
        >{label}</button>
    );
            ...
            >
                <div className="p-4 space-y-4">
                    ...
                    <SliderControl
                        ...
                        onChange={v => onUpdateArrow({ strokeWidth: v })}
            ...
            >
                <div className="p-4 space-y-4">
                    ...
                    <SliderControl
                        label="Scale"
                        value={counter.scale}
                        onChange={v => onUpdateCounter({ scale: v })}
            ...
            >
                <div className="p-4 space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex bg-neutral-700 p-1 rounded-lg">
                            <button onClick={() => onUpdateShape({ shapeType: 'rect' })} className={`flex-1 py-1 text-xs font-medium rounded-md transition-colors ${shape.shapeType === 'rect' ? 'bg-neutral-600 text-white' : 'text-neutral-400'}`}>Rect</button>
                            <button onClick={() => onUpdateShape({ shapeType: 'circle' })} className={`flex-1 py-1 text-xs font-medium rounded-md transition-colors ${shape.shapeType === 'circle' ? 'bg-neutral-600 text-white' : 'text-neutral-400'}`}>Circle</button>
                            <button onClick={() => onUpdateShape({ shapeType: 'triangle' })} className={`flex-1 py-1 text-xs font-medium rounded-md transition-colors ${shape.shapeType === 'triangle' ? 'bg-neutral-600 text-white' : 'text-neutral-400'}`}>Triangle</button>
                        </div>
                    </div>
                    <div className="flex items-center justify-between">
                    ...
                    <SliderControl
                        ...
                        onChange={v => onUpdateShape({ strokeWidth: v })}
            ...
            <div className="p-4">
                {activeTab === 'style' && (
                    <div className="space-y-4">
                        ...
                        <SliderControl
                            label="Size"
                            value={text.fontSizeScale || 1}
                            onChange={v => onUpdateText({ fontSizeScale: v })}
                ...
                {activeTab === 'effects' && (
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <label htmlFor="glass-text-toggle" className="text-sm font-medium text-neutral-300">Glassmorphism</label>
                            <button
                                role="switch"
...
const AnnotationToolbar: React.FC<{
    ...
    const ToolbarButton: React.FC<{
        ...
        <button
            ...
            disabled={disabled}
            title={title}
            className={`p-2.5 rounded-full transition-colors duration-200 ${isActive ? 'bg-blue-600 text-white' :
                disabled
                    ? 'text-neutral-600'
    ...
    return (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center space-x-1 bg-neutral-900/50 backdrop-blur-xl p-1.5 rounded-full shadow-2xl shadow-black/30 border border-white/10 z-50">
            ...
            <ToolbarButton
                ...
                className="disabled:text-neutral-600 text-neutral-300 hover:text-red-400 hover:bg-red-500/20"
            >
...
export default function ImageGenerator() {
    ...
    const [uploadedImages, setUploadedImages] = useState<UploadedImage[]>([{
        ...
        name: 'YourGPT',
        src: 'https://media.discordapp.net/attachments/805778367706169415/1449431887524266215/YourGPT.png?ex=693ee023&is=693d8ea3&hm=afaf79ac4fca22af9b2552fca0ccfe2e97478c9bcd1bb22630c9c397a6913caf&=&format=webp&quality=lossless&width=2950&height=1543'
    }]);
    ...
    const handleArrowAdd = useCallback((canvasKey: number, arrow: Omit<ArrowObject, 'id' | 'type'>) => {
        pushToHistory();
    ...
    const handleTextUpdateWithHistory = useCallback((canvasKey: number, id: string, props: Partial<Omit<TextObject, 'id'>>) => {
        pushToHistory();
    ...
    const handleArrowUpdateWithHistory = useCallback((canvasKey: number, id: string, props: Partial<Omit<ArrowObject, 'id' | 'type'>>) => {
        pushToHistory();
        handleArrowUpdate(canvasKey, id, props);
    }, [pushToHistory, handleArrowUpdate]);

    const handleTextEffectsUpdate = useCallback((props: Partial<TextEffects>) => {
        pushToHistory();
        setTextEffects(prev => ({ ...prev, ...props }));
    }, [pushToHistory]);

    const handleTextSubEffectChange = useCallback((prop: 'shadow' | 'stroke', key: string, value: string | number) => {
        pushToHistory();
        setTextEffects(prev => ({ ...prev, [prop]: { ...prev[prop], [key]: value } }));
    }, [pushToHistory]);

    const handleTextDelete = useCallback((canvasKey: number, id: string) => {
        pushToHistory();
        setAllTexts(prev => {
    ...
    const handleArrowDelete = useCallback((canvasKey: number, id: string) => {
        pushToHistory();
        setAllArrows(prev => {
    ...
    // Counter Handlers
    const handleCounterAdd = useCallback((coords?: { x: number, y: number }) => {
        const activeCanvasKey = activeImageIndex !== null ? activeImageIndex : -1;
        pushToHistory();
    ...
    const handleCounterUpdateWithHistory = useCallback((canvasKey: number, id: string, props: Partial<Omit<CounterObject, 'id' | 'type'>>) => {
        pushToHistory();
        handleCounterUpdate(canvasKey, id, props);
    }, [pushToHistory, handleCounterUpdate]);

    const handleCounterDelete = useCallback((canvasKey: number, id: string) => {
        pushToHistory();
        setAllCounters(prev => {
    ...
    // Redact Handlers
    const handleRedactAdd = useCallback((canvasKeyParam?: number, redact?: Omit<RedactObject, 'id' | 'type'>) => {
        pushToHistory();
    ...
    const handleRedactUpdateWithHistory = useCallback((canvasKey: number, id: string, props: Partial<Omit<RedactObject, 'id' | 'type'>>) => {
        pushToHistory();
        handleRedactUpdate(canvasKey, id, props);
    }, [pushToHistory, handleRedactUpdate]);

    const handleRedactDelete = useCallback((canvasKey: number, id: string) => {
        pushToHistory();
        setAllRedactions(prev => ({
            ...prev,
    ...
    // Shape Handlers
    const handleShapeAdd = useCallback((canvasKeyParam?: number, shape?: Omit<ShapeObject, 'id' | 'type'>) => {
        pushToHistory();
        const activeCanvasKey = canvasKeyParam ?? (activeImageIndex !== null ? activeImageIndex : -1);
    ...
    const handleShapeUpdateWithHistory = useCallback((canvasKey: number, id: string, props: Partial<Omit<ShapeObject, 'id' | 'type'>>) => {
        pushToHistory();
        handleShapeUpdate(canvasKey, id, props);
    }, [pushToHistory, handleShapeUpdate]);

    const handleShapeDelete = useCallback((canvasKey: number, id: string) => {
        pushToHistory();
        setAllShapes(prev => ({
            ...prev,
        ...
        <div className="h-screen overflow-hidden bg-black text-neutral-100 font-sans flex flex-col lg:flex-row">
            ...
            <aside className="w-full lg:w-[380px] xl:w-[400px] bg-neutral-900/70 backdrop-blur-sm p-6 border-r border-neutral-800 overflow-y-auto order-2 lg:order-1">
                <Controls
                    ...
                    backgroundImage={backgroundImage} handleBackgroundUpload={handleBackgroundUpload} removeBackgroundImage={removeBackgroundImage} setBackgroundImage={setBackgroundImage}
                    backgroundEffects={backgroundEffects} setBackgroundEffects={setBackgroundEffects}
                    textEffects={textEffects} setTextEffects={setTextEffects}
                    imageSettings={imageSettings} setImageSettings={setImageSettings}
                    uploadedImages={uploadedImages} activeImageIndex={activeImageIndex} setActiveImageIndex={setActiveImageIndex}
                    handleFileUpload={handleFileUpload} removeUploadedImage={removeUploadedImage} removeAllUploadedImages={removeAllUploadedImages}
            ...
            <main className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 bg-black/50 relative order-1 lg:order-2 overflow-hidden">
                <div className="w-full h-full overflow-auto flex" onClick={() => { setSelection(null); setIsStylePopoverOpen(false); }}>
                            ...
                            <div className={uploadedImages.length > 1 ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 p-4 w-[90vw]" : "p-4 w-[min(80vh,90vw,1200px)]"}>
                                {uploadedImages.map((image, index) => (
                                    <div key={image.id} className="w-full" onClick={e => e.stopPropagation()}>
                                        <ImagePreview ref={el => { previewRefs.current[index] = el; }} canvasKey={index} isActive={activeImageIndex === index} onActivate={() => setActiveImageIndex(index)} previewContainerRef={previewContainerRef} aspectRatio={aspectRatio} backgroundValue={backgroundValue} backgroundImage={backgroundImage} backgroundEffects={backgroundEffects} textEffects={textEffects} uploadedImage={image.src} imageSettings={imageSettings} texts={allTexts[index] || []} arrows={allArrows[index] || []}
                                            counters={allCounters[index] || []} redactions={allRedactions[index] || []} shapes={allShapes[index] || []}
                                            onTextUpdate={(id: string, props: Partial<Omit<TextObject, 'id'>>) => handleTextUpdate(index, id, props)} onTextUpdateWithHistory={(id: string, props: Partial<Omit<TextObject, 'id'>>) => handleTextUpdateWithHistory(index, id, props)} onTextDelete={(id: string) => handleTextDelete(index, id)}
                                            onArrowAdd={(arrow: Omit<ArrowObject, 'id' | 'type'>) => handleArrowAdd(index, arrow)} onArrowUpdate={(id: string, props: Partial<Omit<ArrowObject, 'id' | 'type'>>) => handleArrowUpdate(index, id, props)} onArrowUpdateWithHistory={(id: string, props: Partial<Omit<ArrowObject, 'id' | 'type'>>) => handleArrowUpdateWithHistory(index, id, props)} onArrowDelete={(id: string) => handleArrowDelete(index, id)}
                                            onCounterAdd={handleCounterAdd} onCounterUpdate={(id, props) => handleCounterUpdate(index, id, props)} onCounterUpdateWithHistory={(id, props) => handleCounterUpdateWithHistory(index, id, props)} onCounterDelete={(id) => handleCounterDelete(index, id)}
                                            onRedactAdd={(redact) => handleRedactAdd(index, redact)} onRedactUpdate={(id, props) => handleRedactUpdate(index, id, props)} onRedactUpdateWithHistory={(id, props) => handleRedactUpdateWithHistory(index, id, props)} onRedactDelete={(id) => handleRedactDelete(index, id)}
                                            onShapeAdd={(shape) => handleShapeAdd(index, shape)} onShapeUpdate={(id, props) => handleShapeUpdate(index, id, props)} onShapeUpdateWithHistory={(id, props) => handleShapeUpdateWithHistory(index, id, props)} onShapeDelete={(id) => handleShapeDelete(index, id)}
                                            selection={selection} onSelectObject={(canvasKey: number, itemId: string | null, type: any) => itemId ? setSelection({ canvasKey, itemId, type }) : setSelection(null)} editing={editing} onSetEditing={handleSetEditing} drawingMode={drawingMode} setDrawingMode={setDrawingMode} />
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <div className="p-4 w-[min(80vh,90vw,1200px)]" onClick={e => e.stopPropagation()}>
                                <ImagePreview ref={singlePreviewRef} canvasKey={-1} isActive={true} onActivate={() => { }} previewContainerRef={previewContainerRef} aspectRatio={aspectRatio} backgroundValue={backgroundValue} backgroundImage={backgroundImage} backgroundEffects={backgroundEffects} textEffects={textEffects} uploadedImage={null} imageSettings={imageSettings} texts={allTexts[-1] || []} arrows={allArrows[-1] || []}
                                    counters={allCounters[-1] || []} redactions={allRedactions[-1] || []} shapes={allShapes[-1] || []}
                                    onTextUpdate={(id: string, props: Partial<Omit<TextObject, 'id'>>) => handleTextUpdate(-1, id, props)} onTextUpdateWithHistory={(id: string, props: Partial<Omit<TextObject, 'id'>>) => handleTextUpdateWithHistory(-1, id, props)} onTextDelete={(id: string) => handleTextDelete(-1, id)}
                                    onArrowAdd={(arrow: Omit<ArrowObject, 'id' | 'type'>) => handleArrowAdd(-1, arrow)} onArrowUpdate={(id: string, props: Partial<Omit<ArrowObject, 'id' | 'type'>>) => handleArrowUpdate(-1, id, props)} onArrowUpdateWithHistory={(id: string, props: Partial<Omit<ArrowObject, 'id' | 'type'>>) => handleArrowUpdateWithHistory(-1, id, props)} onArrowDelete={(id: string) => handleArrowDelete(-1, id)}
                                    onCounterAdd={handleCounterAdd} onCounterUpdate={(id, props) => handleCounterUpdate(-1, id, props)} onCounterUpdateWithHistory={(id, props) => handleCounterUpdateWithHistory(-1, id, props)} onCounterDelete={(id) => handleCounterDelete(-1, id)}
                                    onRedactAdd={(redact) => handleRedactAdd(-1, redact)} onRedactUpdate={(id, props) => handleRedactUpdate(-1, id, props)} onRedactUpdateWithHistory={(id, props) => handleRedactUpdateWithHistory(-1, id, props)} onRedactDelete={(id) => handleRedactDelete(-1, id)}
                                    onShapeAdd={(shape) => handleShapeAdd(-1, shape)} onShapeUpdate={(id, props) => handleShapeUpdate(-1, id, props)} onShapeUpdateWithHistory={(id, props) => handleShapeUpdateWithHistory(-1, id, props)} onShapeDelete={(id) => handleShapeDelete(-1, id)}
                                    selection={selection} onSelectObject={(canvasKey: number, itemId: string | null, type: any) => itemId ? setSelection({ canvasKey, itemId, type }) : setSelection(null)} editing={editing} onSetEditing={handleSetEditing} drawingMode={drawingMode} setDrawingMode={setDrawingMode} />
                            </div>
                        )}
                    </div>
                </div>
                <AnnotationToolbar onAddText={handleAddText} onAddCounter={handleCounterAdd} onAddShape={handleShapeAdd} onAddRedact={handleRedactAdd} onUndo={handleUndo} onDeleteSelected={handleDeleteSelected} isObjectSelected={!!selectedObject} canUndo={history.length > 0} isStylePopoverOpen={isStylePopoverOpen} onToggleStylePopover={() => setIsStylePopoverOpen(p => !p)} drawingMode={drawingMode} setDrawingMode={setDrawingMode}>
                    {selectedObject && (
                        <StylePopover
